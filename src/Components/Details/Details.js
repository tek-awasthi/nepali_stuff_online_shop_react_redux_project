    ...
    return (
      <div className="details-page">
        <div className="details-page-header">
          <div
            className="online-shop-title-smaller"
        ...
        <div className="details-page-content">
          ...
          >
            <div style={{ flex: 1 }}>
              <div
                style={{ fontSize: 20, fontWeight: "bold", color: "#4282ad" }}
              >
                Price: Rs {this.state.item.price}{" "}
              </div>
              {this.state.item.inStock ? (
                <span style={{ color: "#0a7f19", marginTop: 5, fontSize: 14 }}>
                  {" "}
                  In Stock
                </span>
              ) : (
                <span style={{ color: "red", marginTop: 5, fontSize: 14 }}>
                  Not in Stock
                </span>
              )}
              <div style={{ marginTop: 35 }}>
                <Button
                  ...
                  onClick={() => {
                    this.props.dispatch(
                      addItemInCart({ ...this.state.item, quantity: 1 })
                    );
                  }}
                >
                  Add to Cart
          ...
          {this.state.relatedItems.length !== 0 && (
            <Slider {...sliderSettings}>
              {this.state.relatedItems.map((item) => {
                return <Item mini={true} key={item.id} item={item} />;
              })}
            </Slider>
          )}
        </div>
